...
# Nutze simple-websocket im Hintergrund
...
@app.route('/')
def index():
    is_host = request.args.get('host', 'false') == 'true'
    if is_host:
...
# WICHTIG FÜR UVICORN: Verpackt Flask in eine asynchrone ASGI-Anwendung
from engineio.middleware import WSGIMiddleware
asgi_app = WSGIMiddleware(socketio, app)
...
# Automatische Wahl des installierten simple-websocket
socketio = SocketIO(app, cors_allowed_origins="*")
...
@app.route('/')
def index():
    # Prüft, ob der Beamer die Seite aufruft (/?host=true)
    is_host = request.args.get('host', 'false') == 'true'
    if is_host:
        return render_template('host.html')
    return render_template('index.html')
